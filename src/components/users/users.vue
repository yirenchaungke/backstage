...
export default {
  ...
  methods: {
    ...
    async changeMgState(val) {
      const res = await this.$http.put(
        `http://47.97.214.102:8888/api/private/v1/users/${val.id}/state/${
          val.mg_state
        }`
      );
    },
    ...
    async editUser() {
      const res = await this.$http.put(
        `http://47.97.214.102:8888/api/private/v1/users/${this.form.id}`,
        this.form
      );
      this.form = {};
      this.dialogFormVisibleEdit = false;
      ...
      this.$confirm("永久删除该用户, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await this.$http.delete(
            `http://47.97.214.102:8888/api/private/v1/users/${val}`
          );
          this.getUserList();
          this.$message({
            type: "success",
    ...
    async userAdd() {
      const res = await this.$http.post(
        `http://47.97.214.102:8888/api/private/v1/users`,
        this.form
      );
      const {
        meta: { status, msg }
    ...
    async getUserList() {
      ...
      const res = await this.$http.get(
        `http://47.97.214.102:8888/api/private/v1/users?query=${
          this.query
